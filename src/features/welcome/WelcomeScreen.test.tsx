...
/**
 * La vitrina de plantillas es un DIÁLOGO, no una etapa de un recorrido: se
 * abre sobre el lanzador, se elige y se cierra. El diálogo se monta por portal
 * en `document.body`, así que las tarjetas no están bajo el `container` del
 * render — se buscan desde el documento.
 */
...
describe('WelcomeScreen template showcase', () => {
  ...
  it('filters templates by category and restores the full list', async () => {
    ...
    await waitFor(() => expect(templateCards().length).toBeLessThan(total));
    const academic = templateCards();
    ...
    await waitFor(() => expect(templateCards().length).toBe(total - academic.length));
    ...
    await waitFor(() => expect(templateCards()).toHaveLength(total));
  });
