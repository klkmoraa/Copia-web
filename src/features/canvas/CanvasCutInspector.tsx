...
export type CutDemand = ({ status: 'available'; ratio: number } & ElasticIndexPaint) | { status: 'unavailable' };

/**
 * El panel del corte (DCL + ecuaciones de equilibrio), extraído tal cual de
 * `StructuralCanvas.tsx`: sólo presentación. `cutDemand`/`cutEquilibrium`
 * siguen calculándose en el componente que sabe de `analysis`/`resultMap` —
 * aquí sólo se pintan.
 */
export interface CanvasCutInspectorProps {
  cut: CutInfo | null;
  cutDemand: CutDemand | null;
...
export const CanvasCutInspector = ({
  cut,
  cutDemand,
  cutEquilibrium,
  hostRef,
